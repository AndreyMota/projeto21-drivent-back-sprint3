...
describe('GET /hotels', () => {
  ...
  describe('when token is valid', () => {
    it('should respond with status 404 if there is no enrollment for the user', async () => {
      const token = generateValidToken(); // Assuming generateValidToken() generates a valid token.
    ...
    it('should respond with status 404 if there is no ticket for the enrollment', async () => {
      const userWithEnrollment = await createUser();
      const token = generateValidToken(userWithEnrollment);
